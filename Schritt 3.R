...
doc_logDT = tibble(doc['days'], doc_logT, doc_logD, doc_logIso)
...
#ggsave(filename = paste0(graphics , '/ZIB-like.png'), width = w, height = w, units= u , dpi = dpi, 
#       plot = ggplot(doc_change, aes(x = days, y = D_New)) +
#          geom_col(fill = 'white', width = .8) + 
#          geom_smooth(aes(x = days, y = D_New), colour = 'yellow', size = 1.5) +
